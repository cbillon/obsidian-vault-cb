...
tags:
  - moodle
---
[github camp-registry](https://github.com/camp-registry)

# RFC: A Community-Governed Plugin Repository for Moodle

[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#rfc-a-community-governed-plugin-repository-for-moodle)

**Status:** Draft for community comment (revision 10) **Author(s):** the camp project — [https://github.com/camp-registry](https://github.com/camp-registry) **Date:** July 2026 **Feedback:** [https://github.com/camp-registry/camp-docs/discussions](https://github.com/camp-registry/camp-docs/discussions)

---

## 1. Summary

[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#1-summary)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#2-background-and-motivation)

### 2.1 What is changing

[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#21-what-is-changing)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#22-why-this-is-structural-not-incidental)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#3-goals)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#non-goals)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#4-design-overview)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#41-the-index-a-git-repository-as-the-source-of-truth)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#42-automated-verification-the-trust-floor)
...
Only submissions passing all checks are merged. This guarantees, for **every** plugin in the repository, including brand-new, unreviewed ones, that _the artifact you install is exactly what is in the public source repository_. Because Moodle plugins are interpreted PHP with no compilation step, this guarantee is cheap for us and for authors, and it is stronger than what the current directory provides.
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#44-trust-tiers)
...
Each tier answers one question: does it exist (0), is someone accountable for it (1), does the artifact provably match its public source (2), have humans read the code (3). Signals that are version-specific or come from outside the registry's own process — for example a professional **security audit** of a particular release — are deliberately _not_ tiers: they are displayed as per-release badges, recording who attested what about which version, without implying the whole plugin carries that guarantee forever.
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#45-distribution-static-and-mirrorable)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#46-privacy-by-design)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#47-disclosure-labels)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#5-security-reporting-and-advisories-third-party-plugins-only)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#51-reporting-channels)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#52-coordinated-disclosure-policy)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#53-signed-advisories-and-revocation)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#54-triage-team-governance)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#6-moodle-integration)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#61-the-composer-channel-moodle-52-composer-managed-sites)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#62-the-client-plugin-everyone-else)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#63-multiple-repositories-and-storefronts)
...
- **Trust is per-repository and never bleeds across.** Each configured repository carries its own credentials and, as TUF lands (§4.3), its own pinned trust root — the analog of importing a vendor's signing key. A tier is an assertion _by its repository_: "Tier 3" from this registry means two community reviewers signed the promotion; from another repository it means whatever that operator's process is. Clients display tiers attributed to their source, and minimum-tier policy is set per repository — a site may demand Tier 3 from the community registry while accepting a contracted partner's repository wholesale, because the contract is the trust basis there.
...
- **The boundary of the model.** This design is for a handful of administrator-chosen repositories. It is _not_ open federation: with hundreds of auto-discovered sources there is no arbiter, and namespace integrity — the registry's core security function — collapses. The client will not discover repositories on its own.
- **Listings can move — in either direction.** With no exclusivity (§3), an author who takes a plugin to another repository simply stops publishing here, and may mark the listing **moved** with a successor pointer. A moved listing keeps everything the community already relies on: published versions stay in the archive, stay installable, and stay covered by advisories; the component name remains the author's (§8) if they return. Clients surface the pointer ("new versions publish at …") so sites go quiet with an explanation, never silently — but _following_ the plugin stays an explicit administrator action (add the repository, re-pin the component), because automatic follow-the-plugin would reopen exactly the takeover hole that source binding closes. In Composer metadata the pointer is emitted as the standard abandoned-with-replacement field, so `composer` warns natively. Dual-listing needs none of this ceremony: both repositories simply list the component, and the site's priority order decides.
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#7-translations)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#8-naming-trademark-and-namespace-policy)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#9-governance)
...
4. **The exit guarantee.** Because the index is Git, the artifacts are static files, all tooling is free software (GPL/Apache), and signing follows open standards, the community can fork the _entire repository_, content, history, and infrastructure included, if governance ever fails. This property is deliberate. We ask to be trusted, but we build so that trust is not required.

## 10. Risks and mitigations

[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#10-risks-and-mitigations)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#11-roadmap)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#12-what-we-are-asking-of-the-community)
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#13-open-questions)

1. Should Tier 3 review target plugins (all versions after first approval, like the current directory) or each version?
2. How should plugin _adoption_ work when a maintainer disappears (the current directory's adoption programme is a genuinely good pattern worth keeping)?
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#appendix-a-glossary)
...
- **Rekor / sigstore:** free public infrastructure (Linux Foundation) providing an append-only, tamper-evident log of signing events. Used by npm, PyPI, and Kubernetes. Lets anyone verify _when_ something was signed and detect after-the-fact tampering or misuse of identities.
...
[](https://github.com/camp-registry/camp-docs/blob/main/rfc-community-plugin-repository.md#appendix-b-sources-for-21)
