...
byline: |-
  Nathan Barry
  June 14, 2026
excerpt: "A while back I wrote about language modeling without neural networks,
  where I generated Shakespeare with an unbounded n-gram model: no weights, no
  training, …"
slurped: 2026-08-31T09:54
title: Can gzip be a language model?
---

A while back I wrote about [language modeling without neural networks](app://obsidian.md/posts/unbounded-n-gram/), where I generated Shakespeare with an unbounded n-gram model: no weights, no training, just counting. Fortuitously, I came across the paper [Language Modeling is Compression](https://arxiv.org/abs/2309.10668), which mentioned the **compression–prediction equivalence**:

> **every prediction model is inherently a compressor, and all compression algorithms are prediction models**.

This led to the natural question: _can `gzip` do language modeling?_[1](#fn:1) No neural network, no learned parameters, nothing. Just the compressor that ships with your operating system. You prime it with a corpus, give it a normal text prompt, and it continues that prompt by searching for the byte sequences that compress best. Here’s some real, unedited output after priming it on tiny Shakespeare:
...
It turns out, kind of? It’s not exactly coherent text, but it clearly _knows_ something about the text. Much more than I expected `gzip` to _know_.[2](#fn:2) So how can a compressor generate this?

## Compression is prediction[#](#compression-is-prediction)
...
## Generating by beam search[#](#generating-by-beam-search)
