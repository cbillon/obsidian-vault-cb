---
link: https://www.jeffgeerling.com/blog/2017/composer-and-drupal-are-still-strange-bedfellows/
site: Jeff Geerling
date: 2017-05-04T04:31
excerpt: >-
  More and more sites are being built in Drupal 8 (over 160,000 as of DrupalCon
  Baltimore 2017!). As developers determine best practices for Drupal 8 site
  builds and deployment, they need to come to terms with Composer. In one of the
  most visible signs that Drupal is 'off the island', many modules are now
  requiring developers to have at least a fundamental grasp of Composer and
  dependency management.

  But even more than that, many developers now use Composer in place of manual
  dependency management or a simpler tools like Drush Make files.
slurped: 2026-08-16T07:55
...
[![DrupalCon Baltimore 2017 - participants sitting and waiting to see the opening Keynote](app://obsidian.md/blog/2017/composer-and-drupal-are-still-strange-bedfellows/drupalcon-baltimore-2017-keynote-wide.jpg)](https://www.flickr.com/photos/comprock/33483313043/in/pool-drupalconbaltimore2017/)

  
_Over 3,000 community members attended DrupalCon Baltimore 2017._  
_(Photo by Michael Cannon)_
...
- [How do I deploy a Drupal codebase if the `vendor` directory isn't in the codebase?](#1)
- [I need to put a library (e.g. CKEditor for Webform) in the `libraries` directory, and not in `vendor`. Is this even possible with Composer?](#2)
- [One frontend library I need to add to my site (for a module, not a theme) is not on Packagist. How can I `composer require` it without it being on Packagist?](#3)
- [What's the best way to start building the codebase for a new Drupal 8 website?](#4)
- [If I have a multisite installation, how can I put certain modules in one `sites/site-a/modules` directory, and others in another `sites/site-b/modules` directory?](#5)
- [Using the command line to install modules is a lot to ask. Isn't there some sort of UI I could use?](#6)
- [What happens if there are dependencies required by two modules or core and a module, and there's a conflict with version requirements?](#7)
- [How can I include libraries and shared modules that I made for my own sites (they're not on Packagist)?](#8)
- [I have a Drush make file for my site. Can I easily convert this to a `composer.json` file?](#9)
- [I set up my Drupal 8 site by downloading archives from Drupal.org and copying them into the codebase. How can I convert my codebase to use a `composer.json` file?](#10)
- [What is the difference between `^` and `~` in version requirements, and should I use one over the other?](#11)
- [When I run `composer update` a lot of weird things happen and everything is updated, even if I don't want it to be updated. Is there a way I can just update one module at a time?](#12)
- [When I run `composer install` on my production server, it runs out of memory. How can I avoid this issue?](#13)
- [For security reasons, I need to verify that the code I'm running in production wasn't hacked or modified. How can I do this if I'm using Composer to manage my dependencies?](#14)
- [I totally screwed up some of the dependencies on my site while I was tinkering with them. Is there any way to 'nuke' all the things in my codebase that Composer manages so I can do a fresh `composer install` and get them back into the correct state (short of deleting my entire codebase and re-cloning my site locally)?](#15)
- [My team constantly battles with `composer.lock` file merge conflicts. How can we avoid this painful experience?](#16)
- [Do you have any other tips for making Composer use easier and more delightful?](#17)
