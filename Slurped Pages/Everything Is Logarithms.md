...
excerpt: Some connections between things, which I have not seen elsewhere. Maybe
  they mean something?
slurped: 2026-06-23T08:41
...
or[3](https://alexkritchevsky.com/2026/05/25/everything-is-logarithms.html#fn:diff)
...
Maybe it is more correct to \(\log_K V\) as really being an object which refers to all possible bases of \(V\) at once (I’m not sure what it’s called. Sort of a frame bundle but with only one base point?) We can give it coordinates: the space \(X = \log_K V\) is parameterizable by coordinates \((X_0, \Lambda)\), where \(X_0 = (\x, \y, \z)\) is an arbitrary ‘origin’ frame and \(\Lambda\) is an arbitrary linear transformation \(\in GL(V)\), the automorphisms of \(V\).[7](https://alexkritchevsky.com/2026/05/25/everything-is-logarithms.html#fn:torsor) I guess we can should just write
