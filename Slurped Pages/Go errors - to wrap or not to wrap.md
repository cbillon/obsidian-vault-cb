---
link: https://rednafi.com/go/to-wrap-or-not-to-wrap/
byline: Redowan Delowar
site: Redowan Delowar
date: 2026-03-07T00:00
excerpt: Exploring the tradeoffs between wrapping errors at every return site versus wrapping only at boundaries, with no definitive answer - just honest tradeoffs for the kind of software I write.
twitter: https://twitter.com/@rednafi
slurped: 2026-05-01T18:42
...
There’s no consensus, and the answer changes depending on the kind of application you’re writing. The [Go 1.13 blog](https://go.dev/blog/go1.13-errors#whether-to-wrap) already covers the mechanics and offers some guidance, but I wanted to collect more evidence of what people are actually doing in the open and share what’s worked for me.

## The problem with bare errors [#](#the-problem-with-bare-errors)
...
## The case for wrapping at every return site [#](#the-case-for-wrapping-at-every-return-site)

Dave Cheney advocated for this in his 2016 talk [Don’t just check errors](https://dave.cheney.net/2016/04/27/dont-just-check-errors-handle-them-gracefully#annotating-errors) . His `pkg/errors` library introduced `errors.Wrap`, which adds a message and a stack trace at the point where the error occurs. The idea is that each function knows what operation it was attempting, and that context is lost if you don’t capture it immediately.

CockroachDB takes this further. They use [cockroachdb/errors](https://github.com/cockroachdb/errors) , a drop-in replacement for the stdlib `errors` package that captures a stack trace at every wrap site:
...
The Terraform AWS provider does the same thing with `fmt.Errorf("...: %w", err)` at every layer. Their [contributor guidelines](https://hashicorp.github.io/terraform-provider-aws/error-handling/#wrap-errors) mandate a consistent format for all resource operations:
...
The [wrapcheck](https://github.com/tomarrell/wrapcheck) linter codifies this as a rule. It doesn’t flag every bare `return err`, only errors that originated from a different package:
...
> 
> – [Peter Bourgon on Go Time #91](https://changelog.com/gotime/91#t=16:22)

But wrapping has costs that only show up as the codebase grows.

## The cost of overwrapping [#](#the-cost-of-overwrapping)

### Messages pile up [#](#messages-pile-up)
...
[Jay Conrod](https://jayconrod.com/posts/116/error-handling-guidelines-for-go#context-and-wrapping) ’s error handling guidelines address this:
...
The [Google Go Style Guide](https://google.github.io/styleguide/go/best-practices.html#error-extra-info) says the same:
...
### `%w` creates contracts you didn’t mean to [#](#w-creates-contracts-you-didnt-mean-to)
...
The [Go 1.13 blog](https://go.dev/blog/go1.13-errors#whether-to-wrap) uses `sql.ErrNoRows` to illustrate this. Say your `LookupUser` function calls `database/sql` internally:
...
The [Go 1.13 blog](https://go.dev/blog/go1.13-errors#whether-to-wrap) is explicit about this:
...
The [Error Values FAQ](https://go.dev/wiki/ErrorValueFAQ#i-am-already-using-fmterrorf-with-v-or-s-to-provide-context-for-an-error-when-should-i-switch-to-w) makes the same point:
...
The [Google Go Style Guide](https://google.github.io/styleguide/go/best-practices.html#error-extra-info) notes that `%w` is appropriate when your package’s API guarantees that certain underlying errors can be unwrapped and checked by callers. If you don’t want to make that guarantee, use `%v`.
...
## `%v` as the conservative default [#](#v-as-the-conservative-default)
...
At system boundaries, the [Google Go Style Guide](https://google.github.io/styleguide/go/best-practices.html#error-extra-info) recommends translating rather than wrapping:
...
## How the stdlib handles errors [#](#how-the-stdlib-handles-errors)

The standard library also uses [sentinel errors](https://dave.cheney.net/2016/04/27/dont-just-check-errors-handle-them-gracefully#:~:text=Sentinel%20errors) and [custom error types](https://dave.cheney.net/2016/04/27/dont-just-check-errors-handle-them-gracefully#:~:text=Error%20types) alongside `%w` and `%v`.
...
Sentinels work when the caller only needs to know _which_ failure occurred. When callers need structured metadata - not just identity - the stdlib uses custom error types. `os.Open` defines a `*fs.PathError` struct and returns it with the operation name, file path, and underlying syscall error as struct fields:
...
Before Go 1.16, `Rows.Scan` used `%v` here, which severed the chain. Custom `Scanner` implementations returning sentinel errors couldn’t be inspected with `errors.Is` by callers. [Issue #38099](https://github.com/golang/go/issues/38099) fixed this by switching to `%w`. But in the same package, internal type conversion errors use `%v` because the underlying `strconv` parse error is an implementation detail callers don’t need to inspect:
...
Kubernetes went through a similar migration. They historically used `%v` for most wrapping, which meant `errors.As` couldn’t traverse the chain. [Issue #123234](https://github.com/kubernetes/kubernetes/issues/123234) tracked the codebase- wide migration from `%v` to `%w`, acknowledging that `%v` may still be preferred in some places “to abstract the implementation details” but that such cases should be rare.
...
## Structured logging as an alternative to wrapping [#](#structured-logging-as-an-alternative-to-wrapping)
...
> 
> – [Dave Cheney on pkg/errors #245](https://github.com/pkg/errors/issues/245#issue-988166855)
...
## How wrapping changes by application type [#](#how-wrapping-changes-by-application-type)
...
> 
> – [Marcel van Lohuizen on Go Time #91](https://changelog.com/gotime/91#t=12:03)
...
## Where I’ve landed [#](#where-ive-landed)
...
- At package boundaries, `fmt.Errorf("doing X: %w", err)` with identifying info (user IDs, item IDs, file paths). The [wrapcheck](https://github.com/tomarrell/wrapcheck) linter can enforce this automatically. Only wrap when you’re adding information the inner error doesn’t already carry.
- At system boundaries (RPCs, database calls, third-party APIs), translate rather than wrap. Map implementation errors into your own [sentinel errors](https://dave.cheney.net/2016/04/27/dont-just-check-errors-handle-them-gracefully#:~:text=Sentinel%20errors) or [custom error types](https://dave.cheney.net/2016/04/27/dont-just-check-errors-handle-them-gracefully#:~:text=Error%20types) so callers depend on your package, not your dependencies. Use `%v` for the fallback path.
