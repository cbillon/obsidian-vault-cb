---
link: https://myzopotamia.dev/setup-local-domains-with-traefik-and-pihole
byline: Radosław Skupnik
excerpt: If you’re running a reverse proxy, you might want to setup easy local domains to access your applications. For exampl...
slurped: 2026-06-03T20:01
...
- [Traefik Reverse Proxy for containerized applications](https://myzopotamia.dev/traefik-reverse-proxy-with-containers)
- [Self-host Pi-hole behind a reverse proxy](https://myzopotamia.dev/selfhost-pihole-on-reverse-proxy)

---

## Setting up custom DNS domains

With the Reverse Proxy setup, we want to create some custom DNS entries (like `pihole.home`, for example) and point them to our Raspberry Pi (or other machine) - then let _Traefik_ route the traffic based on the hostname.
...
My _docker-compose.yml_ file for Pi-hole looks like this:
...
I point them all to the IP address of the Raspberry Pi - _Traefik_ will sort out the traffic based on the hostname.
...
Note: If you want more details on setting up Traefik and its Dynamic Configuration, see [here](https://myzopotamia.dev/traefik-reverse-proxy-with-containers#dynamic-config-files)
