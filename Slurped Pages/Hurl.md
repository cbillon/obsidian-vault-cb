---
link: https://hurl.dev/docs/frequently-asked-questions.html
excerpt: |-
...
## [General](https://hurl.dev/docs/frequently-asked-questions.html#general)

### [Why “Hurl”?](https://hurl.dev/docs/frequently-asked-questions.html#why-hurl)

The name Hurl is a tribute to the awesome [curl](https://curl.haxx.se/), with a focus on the HTTP protocol. While it may have an informal meaning not particularly elegant, [other eminent tools](https://git.wiki.kernel.org/index.php/GitFaq#Why_the_.27Git.27_name.3F) have set a precedent in naming.

### [Yet Another Tool, I already use X](https://hurl.dev/docs/frequently-asked-questions.html#yet-another-tool-i-already-use-x)

We think that Hurl has some advantages compared to similar tools.

Hurl is foremost a command line tool and should be easy to use on a local computer, or in a CI/CD pipeline. Some tools in the same space as Hurl ([Postman](https://www.postman.com/) for instance), are GUI oriented, and we find it less attractive than CLI. As a command line tool, Hurl can be used to get HTTP data (like [curl](https://curl.haxx.se/)), but also as a test tool for HTTP sessions, or even as documentation.

Having a text based [file format](https://hurl.dev/docs/hurl-file.html) is another advantage. The Hurl format is simple, focused on the HTTP domain, can serve as documentation and can be read or written by non-technical people.
...
[Karate](https://github.com/intuit/karate), a tool combining API test automation, mocking, performance-testing, has similar features but offers also much more at a cost of an increased complexity.
...
Finally, with no headless browser and working on the raw HTTP data, Hurl is also really reliable with a very small probability of false positives. Integration tests with tools like [Selenium](https://www.selenium.dev/) can, in this regard, be challenging to maintain.

Just use what is convenient for you. In our case, it’s Hurl!

### [Hurl is build on top of libcurl, but what is added?](https://hurl.dev/docs/frequently-asked-questions.html#hurl-is-build-on-top-of-libcurl-but-what-is-added)

Hurl has two main functionalities on top of [curl](https://curl.haxx.se/):

1. Chain several requests:
    
    With its [captures](https://hurl.dev/docs/capturing-response.html), it enables to inject data received from a response into following requests. [CSRF tokens](https://en.wikipedia.org/wiki/Cross-site_request_forgery) are typical examples in a standard web session.
    
2. Test HTTP responses:
    
    With its [asserts](https://hurl.dev/docs/asserting-response.html), responses can be easily tested.
...
### [Why shouldn’t I use Hurl?](https://hurl.dev/docs/frequently-asked-questions.html#why-shouldnt-i-use-hurl)

If you need a GUI. Currently, Hurl does not offer a GUI version (like [Postman](https://www.postman.com/)). While we think that it can be useful, we prefer to focus for the time-being on the core, keeping something simple and fast. Contributions to build a GUI are welcome.

### [I have a large numbers of tests, how to run just specific tests?](https://hurl.dev/docs/frequently-asked-questions.html#i-have-a-large-numbers-of-tests-how-to-run-just-specific-tests)
...
### [How can I use my Hurl files outside Hurl?](https://hurl.dev/docs/frequently-asked-questions.html#how-can-i-use-my-hurl-files-outside-hurl)
...
### [Can I do calculation within a Hurl file?](https://hurl.dev/docs/frequently-asked-questions.html#can-i-do-calculation-within-a-hurl-file)
...
You can also use [filters](https://hurl.dev/docs/filters.html) to process HTTP responses in asserts and captures.

## [macOS](https://hurl.dev/docs/frequently-asked-questions.html#macos)

### [How can I use a custom libcurl (from Homebrew by instance)?](https://hurl.dev/docs/frequently-asked-questions.html#how-can-i-use-a-custom-libcurl-from-homebrew-by-instance)
