---
link: https://louis.co.nz/2026/03/26/pratt-parsing.html
byline: Louis Baragwanath
site: louisb0
date: 2026-03-26T01:00
excerpt: You already know that a + b * c + d is calculated as a + (b * c) + d. But how do you encode that knowledge precisely enough for a machine to act on it?
slurped: 2026-04-03T19:17
...
The most common solution employed by compilers is to make use of a tree known as an _abstract syntax tree_. In an AST, each operator sits above its operands, and evaluation works bottom-up: resolve the children, then apply the operation.
...
But of course, people (for the most part) don’t write programs as trees. This means we face the problem of _deriving_ this structure from flat text.
...
In the case of decreasing precedence, we repeatedly evaluate the leftmost operator as it is higher precedence - multiplication before addition, addition before comparison, and so on. The first operator sits deepest in the tree, the last the shallowest. The resulting tree is _left-leaning_.
...
You can imagine what happens when precedence is increasing. It’s the exact opposite: the leftmost operator is now the shallowest and the rightmost the deepest, as each operator depends on the result to its right. The tree is _right-leaning_.
...
It depends on the operation. Convention favours a left-to-right evaluation for arithmetic, known as _left associativity_. Some language features favour the opposite: assignment in C, for instance, is right associative.
...
We should refine our definitions accordingly. Given a sequence of operators, let `x_i` be the precedence of the `i`th operator:

- Decreasing: _weakly_ decreasing - `x_i >= x_{i+1}`. This now includes the case of equal precedence.
- Increasing: _strictly_ increasing - `x_i < x_{i+1}`.
...
The left-leaning tree starts at the first place it can: at an operator of equal or lesser precedence. This lower precedence operator must evaluate later than _at least_ the current one, but there could be several prior operators that also must evaluate first. Those operators are found along the spine of the right-leaning tree.
...
Since any expression is just a sequence of these transitions, this is all we need. This walk-back procedure _is_ Pratt parsing.
...
But `if` is only capable of taking that greedy choice _once_ - we need to take it every time, because anything else is incorrect. So, we replace `if` with `while`:
...
We want the opposite for right-associative operators. `a = b = c` should parse as `a = (b = c)`. The second `=` _should_ be consumed by the recursive call, not by the loop. We can achieve this by setting RBP lower than LBP - the recursive child’s precedence threshold is low enough that a consecutive operator still passes the `>` check and gets consumed deeper.
