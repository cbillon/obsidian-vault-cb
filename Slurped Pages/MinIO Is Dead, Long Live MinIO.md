...
byline: Ruohang Feng
site: Vonng
date: 2026-02-14T01:00
excerpt: MinIO’s repo is officially archived and abandoned. And how AI Agents helped bring it back from the dead. This post explains how a community fork restores the admin console and ships binaries via CI/CD pipeline.
tags:
  - minio
slurped: 2026-05-11T09:03
...
If you’re running MinIO, swap `minio/minio` for `pgsty/minio`. Everything else stays the same. ([CVE fixed](https://nvd.nist.gov/vuln/detail/CVE-2025-62506), and the console GUI is back)

---

## The Death Certificate[#](#the-death-certificate)

On December 3, 2025, MinIO announced “maintenance mode” on GitHub. I wrote about it in [MinIO Is Dead](app://obsidian.md/en/db/minio-is-dead/).
...
![archived.webp](app://obsidian.md/db/minio-resurrect/archived.webp)
...
![mermaid-timeline.webp](app://obsidian.md/db/minio-resurrect/mermaid-timeline.webp)
...
## But Open Source Endures[#](#but-open-source-endures)
...
Some background: I maintain [Pigsty](https://pigsty.io/) — a batteries-included PostgreSQL distribution with [460+ extensions](https://pgext.cloud/), cross-built for [14 Linux distros](https://pgext.cloud/os). I also maintain build pipelines for [290](https://pgext.cloud/list/) PG extensions, several [PG forks](https://pigsty.io/docs/pgsql/kernel/), and dozens of [Go Projects](https://pigsty.io/docs/repo/infra/list/) (Victoria, Prometheus, etc.) packaging across all major platforms. Adding one more to the pipeline was a piece of cake.
...
More importantly, [MinIO is an optional module in Pigsty](https://pigsty.io/docs/minio). Many users run it as the default backup repository for PostgreSQL in production. We did consider several [**alternatives**](app://obsidian.md/en/db/minio-alternative), but none were a drop-in replacement for MinIO-based workflows.

[![minio-docs-en.webp](app://obsidian.md/db/minio-resurrect/minio-docs-en.webp)](https://pigsty.io/docs/minio/config/)

We use MinIO ourselves, so keeping the supply chain alive was not optional — **it had to be done.** As early as December 2025, when MinIO announced maintenance mode, I had already built [CVE-patched](https://nvd.nist.gov/vuln/detail/CVE-2025-62506) binaries and switched to them.

[![releases.webp](app://obsidian.md/db/minio-resurrect/releases.webp)](https://github.com/pgsty/minio/releases/tag/RELEASE.2025-12-03T12-00-00Z)
...
### 1. Restored the Admin Console[#](#1-restored-the-admin-console)
...
![gui.webp](app://obsidian.md/db/minio-resurrect/gui.webp)
...
[![console.webp](app://obsidian.md/db/minio-resurrect/console.webp)](https://github.com/pgsty/minio/commit/8630937e7d1c3426ae28508e06f7091d7bde3a49#diff-3295df7234525439d778f1b282d146a4f1ff6b415248aaac074e8042d9f42d63L424)

We put it back.

### 2. Rebuilt Binary Distribution[#](#2-rebuilt-binary-distribution)
...
For native Linux installs, grab RPM/DEB packages from the [GitHub Release](https://github.com/pgsty/minio/releases/tag/RELEASE.2026-02-14T12-00-00Z) page. You can also use [pig](https://github.com/pgsty/pig) (the PG extension package manager) for easy installation, or configure the [**pigsty-infra**](https://pigsty.io/docs/repo/infra) APT/DNF repo to install from it:
...
### 3. Restored Community Edition Docs[#](#3-restored-community-edition-docs)
...
![doc.webp](app://obsidian.md/db/minio-resurrect/doc.webp)
...
### No New Features — Just Supply Chain Continuity[#](#no-new-features--just-supply-chain-continuity)
...
### This Is a Production Build, Not an Archive[#](#this-is-a-production-build-not-an-archive)
...
### I’m willing to Track CVEs and Fix Bugs[#](#im-willing-to-track-cves-and-fix-bugs)

If you run into issues, feel free to report them at [`pgsty/minio`](https://github.com/orgs/pgsty/discussions). I’ll do my best to fix these — but please don’t treat this as a commercial SLA.
...
### Trademark Is Tricky, But We’ll Cross That Bridge When We Come to It[#](#trademark-is-tricky-but-well-cross-that-bridge-when-we-come-to-it)
...
### AI Changed the Game[#](#ai-changed-the-game)

You might ask: can one person really maintain this?

It’s 2026. Things are different now.

[AI coding tools are changing the economics of open-source maintenance](app://obsidian.md/db/try-codex/).
...
- [**MinIO Is Dead**](app://obsidian.md/en/db/minio-is-dead/)
- [**MinIO Is Dead, Are There Alternatives?**](app://obsidian.md/en/db/minio-alternative/)
- [**From AGPL to Apache: Reflections on Pigsty’s License Change**](app://obsidian.md/en/pg/pigsty-relicense/)
- [**MinIO: Promise made, Promise kept**](app://obsidian.md/en/db/minio-promise-kept/)

> [**Originally published in Chinese]**]([https://vonng.com/db/minio-resurrect/db/minio-promise-kept](https://vonng.com/db/minio-resurrect/db/minio-promise-kept))