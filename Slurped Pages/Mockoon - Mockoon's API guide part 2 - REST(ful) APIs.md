---
link: https://mockoon.com/articles/api-guide-what-are-rest-api/
site: Mockoon
...
slurped: 2025-12-22T17:29
title: "Mockoon - Mockoon's API guide part 2: REST(ful) APIs"
tags:
  - api
---

##  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#definition-of-a-rest-or-restful-api)Definition of a REST (or RESTful) API

As we saw in the [previous part of this guide](https://mockoon.com/articles/api-guide-what-are-api/), there are many types of web APIs. What interests us most is one of the most popular implementations, the **[REST, or RESTful, API](https://en.wikipedia.org/wiki/Representational_state_transfer)**. REST stands for **RE**presentational **S**tate **T**ransfer. It's a software architectural style that defines a set of **constraints** used to create standardized APIs. Web APIs adhering to the REST architectural constraints are called **RESTful APIs**.
...
###  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#client-server-architecture)Client-server architecture
...
###  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#uniform-interface)Uniform interface
...
###  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#statelessness)Statelessness
...
###  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#cacheability)Cacheability
...
##  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#overview-of-a-rest-api-call)Overview of a REST API call

We learned that a REST API follows a set of architectural constraints, but let's have a look at the anatomy of a REST API call between a **client** and a **server**. An API call is always composed of a **request** and a **response**.
...
![API call schema with a request and a response](https://mockoon.com/images/articles/api-guide/api-call-request-response.png)

###  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#the-request)The request
...
`POST /api/v2/users HTTP/1.1 Accept: application/json Content-Type: application/json {   "name": "Peter" }`

Or, in other words:

Copy

`METHOD URL HTTP_VERSION HEADER_KEY_1: HEADER_VALUE_1 HEADER_KEY_2: HEADER_VALUE_2 BLANK_LINE BODY`

As we will see in the [third part of this guide](https://mockoon.com/articles/api-guide-rest-api-components/), the **method and URL are always present and mandatory**. They identify the target endpoint requested by the client on the API server. On the contrary, the **headers and body are optional**. However, it's rare to have a request (or a response) without headers.

###  [](https://mockoon.com/articles/api-guide-what-are-rest-api/#the-response)The response
...
`HTTP/1.1 201 Created Content-Type: application/json {   "id": 123456 }`

Or, in other words:

Copy

`HTTP_VERSION STATUS_CODE STATUS_NAME HEADER_KEY: HEADER_VALUE BLANK_LINE BODY`
