---
link: https://mockoon.com/articles/api-guide-rest-api-components/
site: Mockoon
excerpt: "Discover what composes RESTful API request and response: HTTP methods, resource URL, HTTP headers, payload or body"
slurped: 2025-12-22T17:30
title: "Mockoon - Mockoon's API guide part 3: REST APIs components"
tags:
  - api
---

##  [](https://mockoon.com/articles/api-guide-rest-api-components/#components-of-a-rest-api-request-and-response)Components of a REST API request and response

After seeing [how an API call works](https://mockoon.com/articles/api-guide-what-are-rest-api/), with a request and a response, in the second part of this guide, we will now see what are the request's and response's components: the request's **method** and **URL**, the response's **status code**, and the **headers** and **body**, present in both the request and the response.

![REST API call payload components](https://mockoon.com/images/articles/api-guide/rest-api-call-components.png)

###  [](https://mockoon.com/articles/api-guide-rest-api-components/#resource-url-request)Resource URL (request)
...
###  [](https://mockoon.com/articles/api-guide-rest-api-components/#method-request)Method (request)
...
They map to the corresponding CRUD operations: **Create**, **Read**, **Update**, **Delete**. Each method used in combination with a specific URL will result in a different action performed on the data. Let's see some examples:
...
###  [](https://mockoon.com/articles/api-guide-rest-api-components/#status-code-response)Status code (response)
...
Have you noticed the difference between a successful GET (200 OK) and a successful POST (201 Created)? 200 and 201 belong to the same status codes category, "successes", but they also inform on the **nature of the action**. With this variety of statuses, applications can give **more precise feedback** to their users instead of relying on generic success or error messages.
...
They are usually different between request and response, and some headers don't make sense in a request and vice-versa. Among the most well-known, we can enumerate `Content-Type`, `Authorization`, `Accept`, `Content-Length`, `Cache-Control`, etc.
...
`GET http://company.com/api/v2/users HTTP1.1 Accept: application/json Authorization: Bearer 1234567890abcdef`
...
`HTTP/1.1 200 OK Content-Type: application/json Content-Length: 105 [   {    "id": 1,    "username": "john45"  },  {    "id": 2,    "username": "mary12"  } ]`
...
###  [](https://mockoon.com/articles/api-guide-rest-api-components/#body-or-payload)Body or payload
...
You understand now, both request and response can contain a body, but it doesn't make sense in some cases. Sometimes it is even highly discouraged and not part of the HTTP specification. It is the case for a GET request which should not contain a body as you only want to retrieve data from the server, not send it. However, you will find implementations contradicting this.
...
##  [](https://mockoon.com/articles/api-guide-rest-api-components/#how-to-test-api-calls)How to test API calls?
...
##  [](https://mockoon.com/articles/api-guide-rest-api-components/#api-mocking)API mocking

During development, you may encounter blockers that will slow you down. For example, you may want to start connecting your front-end application with an API that your back-end team is currently working on. Or you would like to work with a third-party API (authentication, payment, weather or financial data, etc.) without too much hassle like creating an account or provision authentication tokens.
...
Mockoon is here to help you start integrating APIs in no time by quickly mocking them locally. Head over to our [getting started tutorial](https://mockoon.com/tutorials/getting-started/) to learn how to mock your first API.