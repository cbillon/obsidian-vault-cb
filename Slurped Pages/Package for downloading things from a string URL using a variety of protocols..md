---
link: https://github.com/hashicorp/go-getter
site: GitHub
excerpt: Package for downloading things from a string URL using a variety of protocols. - hashicorp/go-getter
twitter: https://twitter.com/@github
slurped: 2026-05-01T10:01
title: "GitHub - hashicorp/go-getter: Package for downloading things from a string URL using a variety of protocols."
tags:
  - go
---
...
The concept of a _detector_ automatically turns invalid URLs into proper URLs. For example: "github.com/hashicorp/go-getter" would turn into a Git URL. Or "./foo" would turn into a file URL. These are extensible.
...
[](https://github.com/hashicorp/go-getter#installation-and-usage)
...
[](https://github.com/hashicorp/go-getter#security)
...
go-getter contains mitigations for some of these security issues, but should still be used with caution in security-critical contexts. See the available [security options](https://github.com/hashicorp/go-getter#Security-Options) that can be configured to mitigate some of these risks.
...
In some cases, the protocol to use is ambiguous depending on the source URL. For example, "[http://github.com/mitchellh/vagrant.git](http://github.com/mitchellh/vagrant.git)" could reference an HTTP URL or a Git URL. Forced protocol syntax is used to disambiguate this URL.
...
Subdirectory paths may also use filesystem glob patterns. The path must match _exactly one_ entry or go-getter will return an error. This is useful if you're not sure the exact directory name but it follows a predictable naming structure.
...
[](https://github.com/hashicorp/go-getter#checksumming)
...
[](https://github.com/hashicorp/go-getter#unarchiving)
...
[](https://github.com/hashicorp/go-getter#local-files-file)
...
The "scp-style" addresses _cannot_ be used in conjunction with the `ssh://` scheme prefix, because in that case the colon is used to mark an optional port number to connect on, rather than to delimit the path from the host.

### Mercurial (`hg`)

[](https://github.com/hashicorp/go-getter#mercurial-hg)
...
[](https://github.com/hashicorp/go-getter#basic-authentication)
...
[](https://github.com/hashicorp/go-getter#headers)

Optional request headers can be added by supplying them in a custom [`HttpGetter`](https://godoc.org/github.com/hashicorp/go-getter#HttpGetter) (_not_ as query parameters like most other options). These headers will be sent out on every request the getter in question makes.

### S3 (`s3`)

[](https://github.com/hashicorp/go-getter#s3-s3)
...
[](https://github.com/hashicorp/go-getter#using-iam-instance-profiles-with-s3)
...
[](https://github.com/hashicorp/go-getter#using-s3-with-minio)
...
[](https://github.com/hashicorp/go-getter#s3-bucket-examples)
...
- "s3::[http://127.0.0.1:9000/test-bucket/hello.txt?aws_access_key_id=KEYID&aws_access_key_secret=SECRETKEY&region=us-east-2](http://127.0.0.1:9000/test-bucket/hello.txt?aws_access_key_id=KEYID&aws_access_key_secret=SECRETKEY&region=us-east-2)"

### GCS (`gcs`)

[](https://github.com/hashicorp/go-getter#gcs-gcs)

#### GCS Authentication

[](https://github.com/hashicorp/go-getter#gcs-authentication)
...
[](https://github.com/hashicorp/go-getter#gcs-bucket-examples)
...
[](https://github.com/hashicorp/go-getter#gcs-testing)
