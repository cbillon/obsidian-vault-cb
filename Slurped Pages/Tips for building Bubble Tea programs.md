...
- [2. Dump messages to a file](#2-dump-messages-to-a-file)
- [3. Live reload code changes](#3-live-reload-code-changes)
- [4. Use receiver methods on your model judiciously](#4-use-receiver-methods-on-your-model-judiciously)
- [5. Messages are not necessarily received in the order they are sent](#5-messages-are-not-necessarily-received-in-the-order-they-are-sent)
- [6. Build a tree of models](#6-build-a-tree-of-models)
- [7. Layout arithmetic is error-prone](#7-layout-arithmetic-is-error-prone)
- [8. Recovering your terminal](#8-recovering-your-terminal)
- [9. Use teatest for end-to-end tests](#9-use-teatest-for-end-to-end-tests)
- [10. Record demos and screenshots on VHS](#10-record-demos-and-screenshots-on-vhs)
- [11. And more…](#11-and-more)
...
![pug tasks screenshot](app://obsidian.md/tasks.png)
...
If you repeatedly run this program you’ll find it returns `initialized` some of the time and sometimes `uninitialized`. In the latter case, the event loop has already called `View()` before the go routine sets the content to `initialized` (see the [event loop code above](#keepfast)).

Unless there is a good reason to do otherwise, stick to the normal message flow: any changes to the model should be made in `Update()` and returned immediately in the first return value. Straying from this course not only defies the natural order of Bubbletea, it also risks making it slower (see [Keep the event loop fast](#keepfast)).
...
[https://go.dev/play/p/G4-o8F6PsvU](https://go.dev/play/p/G4-o8F6PsvU)
...
User input messages _are_ therefore sent in order. Just as well otherwise entering words into, say, a text input would end up as gibberish.
...
1. Update model directly within `Update()`:
    ...
    This may contradict with [keeping Update fast](#keepfast).
...
![tree of models in PUG](app://obsidian.md/model-tree.svg)
...
![working layout](app://obsidian.md/layout-arithmetic-bad-but-works/screenshot.png)
...
![broken layout](app://obsidian.md/layout-arithmetic-broken/screenshot.png)
...
![fixed layout](app://obsidian.md/layout-arithmetic-good/screenshot.png)
...
![unreset terminal following panic](app://obsidian.md/terminal-not-reset/panic.gif)
