---
link: https://tylercipriani.com/blog/2020/01/12/unix-permissions-for-dummies/
byline: Tyler Cipriani
excerpt: (“Dummies”, AKA, future me)
slurped: 2026-06-01T08:54
title: UNIX Permissions for Dummies - Tyler Cipriani
tags:
  - linux
  - permission
---

(“Dummies”, AKA, _future me_)
...
As such, I’ve dutifully stared at lots of [man pages](https://man7.org/), I’ve done a bunch of DuckDuckGo searches, and I’ve amalgamated basically everything that I think I kinda/sorta _know_ about permissions and dumped it all into this post.
...
> 
> [/u/mistral](https://www.reddit.com/user/mistralol) on reddit points out that the e`x`ecute permission on a directory does NOT allow a user to list a directory's contents. It does allow a user to enter a directory and access its contents; however, without the `r`ead permission a user cannot list the entries in a directory.
> 
> Oddly, the [`chmod` man page](http://man7.org/linux/man-pages/man2/chmod.2.html) calls this ability "`search`".
...
The remaining 9 letters are `r`ead, `w`rite, and e`x`ecute for user that owns the file, file’s group, and other users; same as above.
...
If the setgid bit is set WITHOUT the group executable bit being set; e.g., `rwxrwS---` some file systems use that as an indicator that the file uses [mandatory file locking](https://www.kernel.org/doc/Documentation/filesystems/mandatory-locking.txt). Caveat emptor – the 0th item in the Linux kernel documentation on mandatory file locking is, “Why you should avoid mandatory locking” – this, to me, makes it seems like a bit of a fraught endeavour.

### setgid on a directory

The setgid bit on a directory will cause files and subdirectories created inside the directory to have the same group as the parent directory – even if the user creating the file is _not_ in the parent directory’s group.
...
Under normal conditions anyone with `w`rite permission to a directory could delete files in that directory.
...
If you combine these two functions – you can take the logical `AND` of a bit and another bit’s `NOT`. This is called the _abjunction_:
...
If I now decide: I’d like to have the file’s user _also_ execute the file, I can `OR` the existing permission, with the execute permission (`1`):
...
When adding permissions, `chmod` uses the logical `OR` to combine file permissions with additional permissions you _add_. In the example above, we added the read permission:
...
We could have achieved the same result by specifying the octal mode for `r`ead for the user that owns the file, i.e. `400`:
...
You can also remove permissions from sets of users with `chmod`. For instance, to remove the ability of `a`ll users to e`x`ecute a file we could use `a-x`; i.e.:
...
Another aspect of permissions to consider is the umask of a process. Umask is a per-process value that tells `mkdir`, `open`, and other file-creation calls what permissions to _take-away_.

Umask is defined in POSIX as both a built-in for the shell[2](https://tylercipriani.com/blog/2020/01/12/unix-permissions-for-dummies/#fn2) and a function[3](https://tylercipriani.com/blog/2020/01/12/unix-permissions-for-dummies/#fn3). Both are used to read and set a umask.
...
Basic permissions for file owners and other users of a system were in-place in UNIX from the beginning – [PDP-7 era](https://minnie.tuhs.org/cgi-bin/utree.pl?file=PDP7-Unix) – circa 1970. Owner and other-user permissions actually pre-date UNIX and were a part of Multics. According to [McIlroy](https://minnie.tuhs.org/pipermail/tuhs/2019-August/018193.html) the concepts of a file owner and other users acting on files were common 10 years prior to the first UNIX.

In late 1973, version 4 of UNIX was released. This release is when group file permissions first show up in the [chmod man page](https://minnie.tuhs.org/cgi-bin/utree.pl?file=V4/man/man2/chmod.2).

Oddly (to me anyway) the setuid concept pre-dates the concept of group permissions by 3 years or so – it’s described by [Ritchie](https://web.archive.org/web/20160304043429/http://www.tuhs.org/Archive/PDP-11/Distributions/research/McIlroy_v0/UnixEditionZero.txt) in early UNIX manuals – and goes all the way back to PDP-7 UNIX. Dennis Ritchie was granted a [patent](https://patents.google.com/patent/US4135240A/en) for setuid in 1973.
...
I published this post as a challenge for myself and my understand and as a useful reference manual for dummies – that is to say _me_ after I, once-again, forget all that esoteric shit about _abjunctions_ for the _N_th time.

---

1. [https://unix.stackexchange.com/questions/79395/how-does-the-sticky-bit-work#comment395032_79401](https://unix.stackexchange.com/questions/79395/how-does-the-sticky-bit-work#comment395032_79401)[↩︎](https://tylercipriani.com/blog/2020/01/12/unix-permissions-for-dummies/#fnref1)
    
2. [https://pubs.opengroup.org/onlinepubs/9699919799/utilities/umask.html](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/umask.html)[↩︎](https://tylercipriani.com/blog/2020/01/12/unix-permissions-for-dummies/#fnref2)
    
3. [https://pubs.opengroup.org/onlinepubs/9699919799/functions/umask.html](https://pubs.opengroup.org/onlinepubs/9699919799/functions/umask.html)[↩︎](https://tylercipriani.com/blog/2020/01/12/unix-permissions-for-dummies/#fnref3)