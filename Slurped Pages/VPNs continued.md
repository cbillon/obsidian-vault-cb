---
link: https://fzakaria.com/2020/09/23/vpns-continued
site: Farid Zakaria’s Blog
date: 2020-09-24T06:46
excerpt: "This is part 2 of a post regarding VPNs. Please see here for part 1.
  In the previous post, we discussed the basics of setting up a virtual private
  network; the goal of which was bridging two distinct private subnets. This
  gave us the alluring property of treating separate networks as they were
  colocated and can be accessed easily. However most people are first introduced
  into VPNs for other features such as: anonymity, secrecy & privacy. A common
  example may be to tunnel traffic through a host in another country to disguise
  the origin. For instance, maybe you are in…"
slurped: 2026-08-24T14:30
title: VPNs continued
---

> This is _part 2_ of a post regarding VPNs. Please see [here](https://fzakaria.com/2020/09/20/vpns-from-first-principles) for part 1.

In the [previous post](https://fzakaria.com/2020/09/20/vpns-from-first-principles), we discussed the basics of setting up a virtual private network; the goal of which was bridging two distinct private subnets.
...
### [§](https://fzakaria.com/2020/09/23/vpns-continued#common-setup)Common Setup

In both alternatives, we will want to make some adjustments to our server. We will want to **enable** _ip_forward_ & **disable** _rp_filter_. 11[https://www.kernel.org/doc/Documentation/networking/ip-sysctl.txt](https://www.kernel.org/doc/Documentation/networking/ip-sysctl.txt)  We also want to create an _iptable_ rule to setup a NAT.
...
Now on **the server**, we will introduce an _iptable_ rule to forward all incoming packets from our _tun0_ device to _eth0_, or whatever our physical device is. Furthermore, we will set the target to _MASQUERADE_.
...
### [§](https://fzakaria.com/2020/09/23/vpns-continued#approach-1-routing-rules)Approach #1: routing rules
...
We will use Linux’s support for multiple routing tables,22[http://linux-ip.net/html/routing-tables.html](http://linux-ip.net/html/routing-tables.html)  to accomplish this goal. We will create a new routing table _lametun_, which is referred to when the _source IP_ of the packet is that of our _tun0_ device.
...
### [§](https://fzakaria.com/2020/09/23/vpns-continued#approach-2-network-namespace)Approach #2: network namespace
...
The final _cherry on top_, is to make sure DNS works inside the namespace. For that let’s create a file **/etc/netns/lametun/resolv.conf** with the following contents:
