---
link: https://www.paradedb.com/learn/search-concepts/hybrid-search
date: 2024-10-06T02:00
excerpt: Learn about hybrid search systems that combine lexical and semantic search for comprehensive results.
tags:
  - search
  - hybrid_search
slurped: 2026-02-22T17:33
title: What is Hybrid Search?
---

Hybrid search combines [full-text search](https://www.paradedb.com/learn/search-concepts/full-text-search) and [vector search](https://www.paradedb.com/learn/search-concepts/vector-search) to deliver both precise keyword matching and semantic understanding in a single query. This approach solves the fundamental trade-off between finding exact terms and discovering conceptually related content, making it particularly valuable for applications where users express queries in diverse ways but expect comprehensive, relevant results.

## Why Combine Search Methods?

Each search approach has distinct strengths and weaknesses that make combination valuable:

- [Full-text search](https://www.paradedb.com/learn/search-concepts/full-text-search) relies on exact keyword matching and struggles when users express concepts differently than documents. A search for "automobile" won't find documents about "cars" unless explicit synonyms are configured. Technical terms, product codes, and precise specifications work well, but natural language queries often fail to match semantically related content.
- [Vector search](https://www.paradedb.com/learn/search-concepts/vector-search) understands semantic relationships but may miss critical exact matches. Searching for a specific product model like "iPhone 15 Pro" might return results about "premium smartphones" that are semantically similar but factually incorrect. Product codes, technical specifications, and precise terminology can be overlooked in favor of conceptually related but wrong results.

## Combining Rankings

The core challenge in hybrid search is combining scores from different ranking algorithms. [Full-text search](https://www.paradedb.com/learn/search-concepts/full-text-search) systems like [BM25](https://www.paradedb.com/learn/search-concepts/bm25) produce relevance scores based on term frequency and document statistics, while [vector search](https://www.paradedb.com/learn/search-concepts/vector-search) generates similarity scores from mathematical distance calculations. These scores operate on different scales and can't be directly combined.
...
For a full discussion of RRF, including why it works, how it can be weighted, and a working SQL example, see our [Reciprocal Rank Fusion](https://www.paradedb.com/learn/search-concepts/reciprocal-rank-fusion) article.
...
1. **Text processing:** The query undergoes traditional text analysis (tokenization, stemming, and stopword removal) before matching against the inverted index using algorithms like [BM25](https://www.paradedb.com/learn/search-concepts/bm25)
...
3. **Result fusion:** Both result sets are combined using techniques like [Reciprocal Rank Fusion](https://www.paradedb.com/learn/search-concepts/reciprocal-rank-fusion) to produce a unified ranking
...
- **Computational overhead:** Running both [full-text](https://www.paradedb.com/learn/search-concepts/full-text-search) and [vector search](https://www.paradedb.com/learn/search-concepts/vector-search) simultaneously increases computational requirements and storage needs
...
Hybrid search represents the current state-of-the-art in information retrieval by combining the precision of [full-text search](https://www.paradedb.com/learn/search-concepts/full-text-search) with the semantic understanding of [vector search](https://www.paradedb.com/learn/search-concepts/vector-search).
