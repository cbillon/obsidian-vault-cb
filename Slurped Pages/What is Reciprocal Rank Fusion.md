---
link: https://www.paradedb.com/learn/search-concepts/reciprocal-rank-fusion
date: 2024-10-06T02:00
excerpt: Learn about Reciprocal Rank Fusion (RRF), a technique for combining multiple search result rankings.
tags:
  - postgresql
  - search
slurped: 2026-02-22T17:29
...
WeightedRRF(d)=∑r∈Rwr⋅1k+rankr(d)\text{WeightedRRF}(d) = \sum_{r \in R} w_r \cdot \frac{1}{k + rank_r(d)}

Where

- dd is a document
- RR is the set of ranked lists
- rankr(d)rank_r(d) is the rank of document dd in ranking rr
- wrw_r is the weight for ranking rr (typically between 0 and 1). In non-weighted/traditional RRF this is always 1.
- kk is a constant (typically 60) that controls the fusion behavior
...
2. Calculate RRF scores for each document that appears in any list and multiply by the weight (if used).
...
The constant k=60k=60 is almost always used because it has been empirically shown to work well across different datasets.

kk can be tuned based on your specific use case and data characteristics. Lower values (20-40) will give top results more influence, higher values (80-100) will give a more gradual contribution difference.
...
**Hybrid Search Systems** Combining lexical search ([BM25](https://www.paradedb.com/learn/search-concepts/bm25)) with semantic vector search:
