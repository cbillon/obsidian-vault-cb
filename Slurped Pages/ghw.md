---
link: https://github.com/jaypipes/ghw/blob/main/README.md
site: GitHub
excerpt: Go HardWare discovery/inspection library. Contribute to jaypipes/ghw
  development by creating an account on GitHub.
twitter: https://twitter.com/@github
slurped: 2026-04-19T14:59
title: ghw/README.md at main · jaypipes/ghw
---

## `ghw` - Go HardWare discovery/inspection library

[](https://github.com/jaypipes/ghw/blob/main/README.md#ghw---go-hardware-discoveryinspection-library)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#design-principles)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#inspecting--monitoring)

`ghw` is a tool for gathering information about your hardware's **capacity** and **capabilities**.

It is important to point out that `ghw` does **NOT** report information that is temporary or variable. It is **NOT** a system monitor nor is it an appropriate tool for gathering data points for metrics that change over time. If you are looking for a system that tracks **usage** of CPU, memory, network I/O or disk I/O, there are plenty of great open source tools that do this! Check out the [Prometheus project](https://prometheus.io/) for a great example.

## Usage

[](https://github.com/jaypipes/ghw/blob/main/README.md#usage)
...
- [`ghw.CPU()`](https://github.com/jaypipes/ghw/blob/main/README.md#cpu)
- [`ghw.Memory()`](https://github.com/jaypipes/ghw/blob/main/README.md#memory)
- [`ghw.Block()`](https://github.com/jaypipes/ghw/blob/main/README.md#block-storage) (block storage)
- [`ghw.Topology()`](https://github.com/jaypipes/ghw/blob/main/README.md#topology) (processor architecture, NUMA topology and memory cache hierarchy)
- [`ghw.Network()`](https://github.com/jaypipes/ghw/blob/main/README.md#network)
- [`ghw.PCI()`](https://github.com/jaypipes/ghw/blob/main/README.md#pci)
- [`ghw.GPU()`](https://github.com/jaypipes/ghw/blob/main/README.md#gpu) (graphical processing unit)
- [`ghw.Accelerator()`](https://github.com/jaypipes/ghw/blob/main/README.md#accelerator) (processing accelerators, AI)
- [`ghw.Chassis()`](https://github.com/jaypipes/ghw/blob/main/README.md#chassis)
- [`ghw.BIOS()`](https://github.com/jaypipes/ghw/blob/main/README.md#bios)
- [`ghw.Baseboard()`](https://github.com/jaypipes/ghw/blob/main/README.md#baseboard)
- [`ghw.Product()`](https://github.com/jaypipes/ghw/blob/main/README.md#product)

### CPU

[](https://github.com/jaypipes/ghw/blob/main/README.md#cpu)
...
- `ghw.ProcessorCore.ID` is the `uint32` identifier that the host gave this core. Note that this does _not_ necessarily equate to a zero-based index of the core within a physical package. For example, the core IDs for an Intel Core i7 are 0, 1, 2, 8, 9, and 10
- `ghw.ProcessorCore.TotalHardwareThreads` is the number of hardware threads associated with the core
- `ghw.ProcessorCore.LogicalProcessors` is an array of ints representing the logical processor IDs assigned to any processing unit for the core. These are sometimes called the "thread siblings". Logical processor IDs are the _zero-based_ index of the processor on the host and are _not_ related to the core ID.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#memory)
...
- `ghw.MemoryInfo.Modules` is an array of pointers to `ghw.MemoryModule` structs, one for each physical [DIMM](https://en.wikipedia.org/wiki/DIMM). Currently, this information is only included on Windows, with Linux support [planned](https://github.com/jaypipes/ghw/pull/171#issuecomment-597082409).
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#physical-versus-usable-memory)

There has been [some](https://github.com/jaypipes/ghw/pull/171) [confusion](https://github.com/jaypipes/ghw/issues/183) regarding the difference between the total physical bytes versus total usable bytes of memory.

Some of this confusion has been due to a misunderstanding of the term "usable". As mentioned [above](https://github.com/jaypipes/ghw/blob/main/README.md#inspection!=monitoring), `ghw` does inspection of the system's capacity.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#block-storage)
...
- `ghw.Disk.WWN` contains a string with the disk's [World Wide Name](https://en.wikipedia.org/wiki/World_Wide_Name)
...
- `ghw.Partition.Label` contains the label for the partition itself. On Linux systems, this is derived from the `ID_PART_ENTRY_NAME` [udev](https://en.wikipedia.org/wiki/Udev) entry for the partition.
- `ghw.Partition.FilesystemLabel` contains the label for the filesystem housed on the partition. On Linux systems, this is derived from the `ID_FS_NAME` [udev](https://en.wikipedia.org/wiki/Udev) entry for the partition.
...
- `ghw.Partition.UUID` is a string containing the partition UUID on Linux and MacOS, and the `VolumeSerialNumber` on Windows (e.g. "A8C3D032"). On Linux systems, this is derived from the `ID_PART_ENTRY_UUID` [udev](https://en.wikipedia.org/wiki/Udev) entry for the partition.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#topology)

> **NOTE**: Topology support is currently Linux-only. Windows support is [planned](https://github.com/jaypipes/ghw/issues/166).
...
`ghw.MemoryArea` describes a collection of _physical_ RAM on the host.

In the simplest and most common case, all system memory fits in a single memory area. In more complex host systems, like [NUMA systems](https://en.wikipedia.org/wiki/Non-uniform_memory_access), many memory areas may be present in the host system (e.g. one for each NUMA cell).
...
See above in the [CPU](https://github.com/jaypipes/ghw/blob/main/README.md#cpu) section for information about the `ghw.ProcessorCore` struct and how to use and query it.
...
- `ghw.NetworkInfo.NICs` is an array of pointers to `ghw.NIC` structs, one for each network interface controller found for the system
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#pci)
...
> **NOTE**: Parsing of the PCI-IDS file database is provided by the separate [github.com/jaypipes/pcidb library](http://github.com/jaypipes/pcidb). You can read that library's README for more information about the various structs that are exposed on the `ghw.PCIInfo` struct.
...
> **NOTE**: Older versions (pre-`v0.9.0`) erroneously referred to the `Device` field as the `Slot` field. As noted by [@pearsonk](https://github.com/pearsonk) in [#220](https://github.com/jaypipes/ghw/issues/220), this was a misnomer.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#finding-a-pci-device-by-pci-address)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#gpu)
...
- `ghw.GPUInfo.GraphicCards` is an array of pointers to `ghw.GraphicsCard` structs, one for each graphics card found for the system
...
**NOTE**: You can [read more](https://github.com/jaypipes/ghw/blob/main/README.md#pci) about the fields of the `ghw.PCIDevice` struct if you'd like to dig deeper into PCI subsystem and programming interface information

**NOTE**: You can [read more](https://github.com/jaypipes/ghw/blob/main/README.md#topology) about the fields of the `ghw.TopologyNode` struct if you'd like to dig deeper into the NUMA/topology subsystem

### Accelerator

[](https://github.com/jaypipes/ghw/blob/main/README.md#accelerator)
...
- `ghw.AcceleratorInfo.Devices` is an array of pointers to `ghw.AcceleratorDevice` structs, one for each processing accelerator card found for the system.
...
- `ghw.AcceleratorDevice.PCIDevice` is a pointer to a `ghw.PCIDevice` struct. describing the processing accelerator card. This may be `nil` if no PCI device information could be determined for the card.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#chassis)
...
- `ghw.ChassisInfo.Type` is a string with the chassis type _code_
...
You can ignore them or use the [Disabling warning messages](https://github.com/jaypipes/ghw/blob/main/README.md#disabling-warning-messages) feature to quiet things down.

### BIOS

[](https://github.com/jaypipes/ghw/blob/main/README.md#bios)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#baseboard)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#product)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#advanced-usage)

### Disabling warning messages

[](https://github.com/jaypipes/ghw/blob/main/README.md#disabling-warning-messages)

When `ghw` isn't able to retrieve some information, it may print certain warning messages to `stderr`. To disable these warnings, set the `GHW_DISABLE_WARNINGS` environs variable:
...
You can disable warning programmatically using the `WithDisableWarnings` modifier:
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#controlling-log-output)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#change-the-log-level)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#use-logfmt-output-format)

To use the [logfmt](https://www.cloudbees.com/blog/logfmt-a-log-format-thats-easy-to-read-and-write) standard log output format, set the `GHW_LOG_LOGFMT` envrions variable:
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#provide-a-custom-logger)

You can programmatically override the logger that `ghw` uses with the `WithLogger` modifier. You pass in an instance of `slog.Logger`, like this example that shows how to use a simple logger with colored log output:
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#overriding-the-root-mountpoint-ghw-uses)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#serialization-to-json-or-yaml)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#overriding-a-specific-mountpoint-linux-only)
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#reading-hardware-information-from-a-ghw-snapshot-linux-only)

The `ghwc snapshot` command creates a snapshot of a host's hardware information.

Use the `ghwc -s` flag to supply a path to a snapshot to read with the `ghwc` command-line program.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#creating-snapshots)

You can create `ghw` snapshots using the `ghwc snapshot` command or programmatically using the `pkg/snapshot` package.
...
[](https://github.com/jaypipes/ghw/blob/main/README.md#calling-external-programs)
...
> **WARNING**: on all platforms, disabling external tools make ghw return less data. Unless noted otherwise, there is _no fallback flow_ if external tools are disabled. On MacOSX/Darwin, disabling external tools disables block support entirely

## Developers

[](https://github.com/jaypipes/ghw/blob/main/README.md#developers)

[Contributions](https://github.com/jaypipes/ghw/blob/main/CONTRIBUTING.md) to `ghw` are welcomed! Fork the repo on GitHub and submit a pull request with your proposed changes. Or, feel free to log an issue for a feature request or bug report.