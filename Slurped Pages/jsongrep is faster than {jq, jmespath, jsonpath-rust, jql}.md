---
link: https://micahkepe.com/blog/jsongrep/
site: Micah's Secret Blog
excerpt: An introduction to the jsongrep tool, a technical explanation of its DFA-based search engine, and performance results against popular JSON query tools.
slurped: 2026-03-28T09:27
...
In this post I'll first show you the tool, then explain _why_ it's fast (conceptually), then _how_ it's fast (the automata theory), and finally _prove_ it (benchmarks).
...
- [A Whirlwind Tour of jsongrep](https://micahkepe.com/blog/jsongrep/#a-whirlwind-tour-of-jsongrep)
- [The jsongrep Pitch](https://micahkepe.com/blog/jsongrep/#the-jsongrep-pitch)
- [The jsongrep Anti-Pitch](https://micahkepe.com/blog/jsongrep/#the-jsongrep-anti-pitch)
- [jsongrep's DFA-Based Query Engine](https://micahkepe.com/blog/jsongrep/#jsongrep-s-dfa-based-query-engine)
    - [The Pipeline at a Glance](https://micahkepe.com/blog/jsongrep/#the-pipeline-at-a-glance)
    - [Parsing the Query](https://micahkepe.com/blog/jsongrep/#parsing-the-query)
    - [JSON as a Tree](https://micahkepe.com/blog/jsongrep/#json-as-a-tree)
    - [Constructing the NFA (Glushkov's Algorithm)](https://micahkepe.com/blog/jsongrep/#constructing-the-nfa-glushkov-s-algorithm)
    - [Determinizing: NFA to DFA (Subset Construction)](https://micahkepe.com/blog/jsongrep/#determinizing-nfa-to-dfa-subset-construction)
    - [Searching: DFS with DFA Transitions](https://micahkepe.com/blog/jsongrep/#searching-dfs-with-dfa-transitions)
- [Benchmarking Methodology](https://micahkepe.com/blog/jsongrep/#benchmarking-methodology)
    - [Datasets](https://micahkepe.com/blog/jsongrep/#datasets)
    - [Compared Tools](https://micahkepe.com/blog/jsongrep/#compared-tools)
    - [Benchmark Groups](https://micahkepe.com/blog/jsongrep/#benchmark-groups)
    - [Equivalent Queries](https://micahkepe.com/blog/jsongrep/#equivalent-queries)
    - [Fairness Considerations](https://micahkepe.com/blog/jsongrep/#fairness-considerations)
- [Benchmark Results](https://micahkepe.com/blog/jsongrep/#benchmark-results)
    - [Document Parse Time](https://micahkepe.com/blog/jsongrep/#document-parse-time)
    - [Query Compile Time](https://micahkepe.com/blog/jsongrep/#query-compile-time)
    - [Search Time](https://micahkepe.com/blog/jsongrep/#search-time)
    - [End-to-End Search Time](https://micahkepe.com/blog/jsongrep/#end-to-end-search-time)
- [Links](https://micahkepe.com/blog/jsongrep/#links)
- [References](https://micahkepe.com/blog/jsongrep/#references)
...
## [A Whirlwind Tour of `jsongrep`](#a-whirlwind-tour-of-jsongrep)
...
![Example of some of jsongrep's search query features in practice.](app://obsidian.md/images/jg-quick-examples.png)
...
## [The `jsongrep` Pitch](#the-jsongrep-pitch)

**JSON documents are trees:** objects and arrays branch, scalars are leaves, and keys and indices label the edges. Querying a JSON document is really about describing _paths_ through this tree. `jsongrep` takes this observation literally: its query language is a **regular language** over the alphabet of keys and indices. Think regular expressions, but instead of matching characters in a string, you're matching edges in a tree.
...
This is the key difference from tools like `jq`, `jmespath`, or `jsonpath-rust`. Those tools _interpret_ path expressions: at each node in the JSON tree, they evaluate the query, check predicates, and recursively descend into matching branches. If a query involves recursive descent (`..` or `$..`), these tools may revisit subtrees or maintain worklists. `jsongrep` does something fundamentally different-- it **compiles** the query into a DFA _before_ it ever looks at the JSON, then walks the document tree exactly once, taking a single $O(1)$ state transition at each edge. No interpretation, no backtracking, one pass.

As a consequence, `jsongrep` is fast-- like **really fast**:

![End-to-end search performance comparison over the xlarge (~190 MB) dataset.](app://obsidian.md/images/e2e-xlarge.png)

End-to-end search performance comparison over the xlarge (~190 MB) dataset.

---

## [The `jsongrep` Anti-Pitch](#the-jsongrep-anti-pitch)

Again borrowing from the `ripgrep` blog post, here's an "anti-pitch" for `jsongrep`:

- `jsongrep` is not as ubiquitous (_yet_) as `jq`. `jq` is the go-to for JSON querying, filtering, and [transductions](https://en.wikipedia.org/wiki/Finite-state_transducer).
    
- The query language is deliberately less expressive than `jq`'s. `jsongrep` is a _search_ tool, not a _transformation_ tool-- it finds values but doesn't compute new ones. There are no filters, no arithmetic, no string interpolation.
...
## [`jsongrep`'s DFA-Based Query Engine](#jsongrep-s-dfa-based-query-engine)
...
1. **Parse the JSON document** into a tree via [`serde_json_borrow`](https://crates.io/crates/serde_json_borrow) (zero-copy).
2. **Parse the user's query** into a [`Query`](https://docs.rs/jsongrep/latest/jsongrep/query/ast/enum.Query.html) AST.
3. **Construct an NFA** from the query via Glushkov's construction algorithm[1](#fn-1).
4. **Determinize the NFA into a DFA** via subset construction[2](#fn-2)
...
### [Parsing the Query](#parsing-the-query)
...
![Sample JSON document as a tree.](app://obsidian.md/images/sample-json-tree.png)
...
### [Constructing the NFA (Glushkov's Algorithm)](#constructing-the-nfa-glushkov-s-algorithm)
...
**2. Compute the First and Last sets.** The _First_ set contains positions that can appear at the start of a match; the _Last_ set contains positions that can appear at the end. For a simple sequence, First = {first element} and Last = {last element}:

- $\textit{First} = \set{1}$ (`roommates`)
- $\textit{Last} = \set{3}$ (`name`)

**3. Compute the Follows set.** For each position _i_, Follows(_i_) is the set of positions that can immediately follow _i_ in a valid match. For a simple sequence, each position follows the one before it:
...
### [Determinizing: NFA to DFA (Subset Construction)](#determinizing-nfa-to-dfa-subset-construction)

An NFA can be in **multiple states simultaneously**-- on a given input, it may have several valid transitions. This is fine theoretically but bad for performance: simulating an NFA means tracking a _set_ of active states at each step. A DFA, by contrast, is in exactly **one state** at all times, meaning each transition is an O(1) table lookup. Importantly, Rabin and Scott showed that every NFA can be turned into an equivalent DFA[2](#fn-2).

The standard algorithm for converting an NFA to a DFA is **subset construction** (also called the powerset construction). The idea is simple: each DFA state corresponds to a _set_ of NFA states. The algorithm explores all reachable sets via breadth-first search:
...
### [Searching: DFS with DFA Transitions](#searching-dfs-with-dfa-transitions)
...
Every node is visited **at most once**, and each transition is an $O(1)$ table lookup. The total search time is **O(n)** where _n_ is the number of nodes in the JSON tree. No backtracking, no interpretation overhead.
...
2. **`query_compile`** -- Query compilation cost only. jsongrep must build an AST _and_ construct a DFA upfront; other tools may have cheaper (or no) compile steps. This is the price jsongrep pays for fast search.
...
### [Document Parse Time](#document-parse-time)

![Document parse times on xlarge dataset across all tools.](app://obsidian.md/images/document-parse-xlarge.png)
...
### [Query Compile Time](#query-compile-time)

As expected, `jsongrep` takes time to compile the different queries and this is its largest cost:

![jsongrep query compile time.](app://obsidian.md/images/query-compile-jsongrep-violin.png)

jsongrep query compile time.

Compare this to the compile time of `jmespath` (an order of magnitude faster):

![jmespath query compile time.](app://obsidian.md/images/jmespath-query-compile-violin.png)
...
![Searches times on xlarge dataset across all tools.](app://obsidian.md/images/query_search_xlarge.png)
...
### [End-to-End Search Time](#end-to-end-search-time)
...
1. Glushkov, V. M. “THE ABSTRACT THEORY OF AUTOMATA.” Russian Mathematical Surveys 16, no. 5 (1961): 1–53. [https://doi.org/10.1070/RM1961v016n05ABEH004112](https://doi.org/10.1070/RM1961v016n05ABEH004112). [↩](#fr-1-1)
    
2. Rabin, M. O., and D. Scott. “Finite Automata and Their Decision Problems.” IBM Journal of Research and Development, vol. 3, no. 2, Apr. 1959, pp. 114–25. ACM.org, [https://dl.acm.org/doi/10.1147/rd.32.0114](https://dl.acm.org/doi/10.1147/rd.32.0114). [↩](#fr-2-1) [↩2](#fr-2-2)