---
link: https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/
...
excerpt: >-
  ...
  and Backups.
slurped: 2026-04-08T14:38
title: "rsync, article 1: Scenarios"
---

Table of contents

- [Scenario: DokuWiki transfers using rsync](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#dokuwiki-transfers-using-rsync)
- [Scenario: Software deployment using rsync](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#software-deployment-using-rsync)
- [Scenario: Backups using rsync](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#backups-using-rsync)
    - [Incremental backups](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#incremental-backups)
    - [Limitation: file system compatibility](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#limitation-file-system-compatibility)
    - [Downside: slow bulk operations (disk usage, deletion)](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#downside-slow-bulk-operations-disk-usage-deletion)
    - [Backup transport (SSH) and scheduling](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#backup-transport-ssh-and-scheduling)
- [Next up](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#next-up)

This post is the first article in a series of blog posts about rsync, [see the Series Overview](https://michael.stapelberg.ch/posts/2022-06-18-rsync-overview/).

To motivate why it makes sense to look at rsync, I present three scenarios for which I have come to appreciate rsync: [DokuWiki transfers](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#dokuwiki-transfers-using-rsync), [Software deployment](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#software-deployment-using-rsync) and [Backups](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/#backups-using-rsync).
...
A DokuWiki installation is exactly the kind of file tree that [`scp(1)`](https://manpages.debian.org/scp.1) cannot efficiently transfer (too many small files), but [`rsync(1)`](https://manpages.debian.org/rsync.1) can! The `rsync` transfer only takes a few seconds, no matter if it’s a full download (can be simpler for batch jobs) or an incremental synchronization (more efficient for regular synchronizations like backups).
...
`rsync` is a great fit here, as it transfers many small files (static assets and templates) efficiently, only transfers the binaries that actually changed, and doesn’t mind if the binary file it’s uploading is currently running (contrary to [`scp(1)`](https://manpages.debian.org/scp.1) , for example).

To illustrate how such a script could look like, here’s my push script for [Debian Code Search](https://codesearch.debian.net/):
...
To back up machines onto my [network storage PC](https://michael.stapelberg.ch/posts/2019-10-23-nas/)’s file system, I ended up with a [hand-written rsync wrapper script](https://github.com/stapelberg/zkj-nas-tools/blob/85e445a284c89590d595a52e16cb6dd652b1388e/dornroeschen/backup-remote.pl) that copies the full file system of each machine into dated directory trees:
...
To revert my `~/.zshrc` to an older version, I can [`scp(1)`](https://manpages.debian.org/scp.1) the file:

```
midna% scp storage2:/srv/backup/midna/2022-05-27/home/michael/configfiles/zshrc ~/configfiles/zshrc
```

To compare a whole older source tree, I can mount it using [`sshfs(1)`](https://manpages.debian.org/sshfs.1) :
...
To check the de-duplication level, we can use [`du(1)`](https://manpages.debian.org/du.1) , first on a single directory:
...
For example, if you use [POSIX ACLs](https://help.ubuntu.com/community/FilePermissionsACLs) or [Extended attributes](https://wiki.archlinux.org/title/File_permissions_and_attributes#Extended_attributes) (possibly for [Capabilities](https://wiki.archlinux.org/title/Capabilities) or [SELinux](https://wiki.archlinux.org/title/SELinux)), you need to ensure that your backup file system has these features enabled, and that you are using [`rsync(1)`](https://manpages.debian.org/rsync.1) ’s `--xattrs` (or `-X` for short) option.

This can turn from a pitfall into a dealbreaker as soon as multiple operating systems are involved. For example, the `rsync` version on macOS has [Apple-specific code](https://github.com/apple-oss-distributions/rsync/blob/aa4e500aa53b9417014c718a5ff0e29215f08e48/rsync/generator.c#L1447) to work with Apple [resource forks](https://en.wikipedia.org/wiki/Resource_fork) and other extended attributes. It’s not clear to me whether macOS `rsync` can send files to Linux `rsync`, restore them, and end up with the same system state.
...
Even merely calculating the disk space usage of each directory tree is a painfully slow operation. I tried using stateful disk usage tools like [duc](http://duc.zevv.nl/), but it [didn’t work reliably](https://github.com/zevv/duc/issues/240) on my backups.

In practice, I found that for tracking down large files, using [`ncdu(1)`](https://manpages.debian.org/ncdu.1) on any recent backup typically quickly shows the large file. In one case, I found `var/lib/postgresql` to consume many gigabytes. I excluded it in favor of using [`pg_dump(1)`](https://manpages.debian.org/pg_dump.1) , which resulted in much smaller backups!
...
(The [corresponding `Dockerfile` can be found in my Gigabit NAS article](https://michael.stapelberg.ch/posts/2016-11-21-gigabit-nas-coreos/#dockerfiles-rrsync-and-samba).)

To trigger such an SSH-protected `rsync` transfer remotely, I’m using a small custom scheduling program called [dornröschen](https://github.com/stapelberg/zkj-nas-tools/tree/master/dornroeschen). The program arranges for all involved machines to be powered on (using [Wake-on-LAN](https://en.wikipedia.org/wiki/Wake-on-LAN)) and then starts `rsync` via _another operation-restricted SSH connection_.

You could easily replace this with a cron job if you don’t care about WOL.

The architecture looks like this:

![backup architecture](https://michael.stapelberg.ch/posts/2022-06-18-rsync-article-1-scenarios/2022-05-29-backup-architecture.svg)

The operation-restricted SSH connection on each backup host is configured in SSH’s [`authorized_keys(5)`](https://manpages.debian.org/authorized_keys.5) config file:
...
The second article in this series is [rsync, article 2: Surroundings](https://michael.stapelberg.ch/posts/2022-07-02-rsync-surroundings/). Now that we know what to use rsync for, how can we best integrate rsync into monitoring and alerting, and on which operating systems does it work?
